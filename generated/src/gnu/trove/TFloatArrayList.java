...
public class TFloatArrayList implements Serializable, Cloneable {
    ...
    protected transient float[] _data; // null means the array list was just created
    ...
    public void ensureCapacity(int capacity) {
        if (_data == null) {
            _data = new float[Math.max(DEFAULT_CAPACITY, capacity)];
        }
    ...
    public float max() {
        ...
        for (int i = _pos - 1; i-- > 0;) {
            max = (float) Math.max(max, _data[_pos]);
        }
        return max;
    }
    ...
    public float min() {
        ...
        for (int i = _pos - 1; i-- > 0;) {
            min = (float) Math.min(min, _data[_pos]);
        }
        return min;
    }
